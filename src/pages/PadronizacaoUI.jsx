...
import StatusFinalEtapa4_100 from '@/components/sistema/StatusFinalEtapa4_100';
  ...
  return (
    <div className="w-full h-full p-6 space-y-6 flex flex-col overflow-auto">
      <Tabs defaultValue="dashboard" className="w-full flex-1 flex flex-col">
         <TabsList className="bg-white border shadow-sm w-full flex-shrink-0 grid grid-cols-6">
            <TabsTrigger value="dashboard" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
              <Layout className="w-4 h-4 mr-2" />
              Dashboard
            </TabsTrigger>
            <TabsTrigger value="status3" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white">
              <CheckCircle className="w-4 h-4 mr-2" />
              Etapa 3
            </TabsTrigger>
            <TabsTrigger value="status4" className="data-[state=active]:bg-green-600 data-[state=active]:text-white">
         ...
         <TabsContent value="dashboard" className="w-full flex-1 overflow-auto">
          ...
          <TabsContent value="status3" className="w-full flex-1 overflow-auto">
            <StatusFinalEtapa3_100 />
          </TabsContent>

          <TabsContent value="status4" className="w-full flex-1 overflow-auto">
            <StatusFinalEtapa4_100 />
          </TabsContent>

          <TabsContent value="certificado" className="w-full flex-1 overflow-auto">
...
// Certificado Etapa 4
function CertificadoEtapa4() {
  return (
    <Card className="border-4 border-green-500 bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 shadow-2xl w-full max-w-4xl">
      <CardContent className="p-8">
        <div className="text-center space-y-6">
          <div className="relative inline-block">
            <div className="absolute inset-0 bg-yellow-400 rounded-full blur-2xl opacity-30 animate-pulse"></div>
            <div className="relative w-32 h-32 mx-auto bg-gradient-to-br from-green-400 via-emerald-500 to-green-600 rounded-full flex items-center justify-center shadow-xl">
              <Trophy className="w-20 h-20 text-white" />
            </div>
          </div>
          ...
          <div>
            ...
            <p className="text-2xl text-green-700 font-semibold">
              Financeiro Unificado & Rastreável 100%
            </p>
          </div>
          ...
          <div className="bg-white/70 backdrop-blur rounded-xl p-6 border-2 border-green-300">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: 'Caixa Central', valor: '100%' },
                { label: 'Detalhes Pagamento', valor: '100%' },
                { label: 'Estágios Cartão', valor: '100%' },
                { label: 'Conciliação Lote', valor: '100%' },
                { label: 'IA Anomalias', valor: '100%' },
                { label: 'Auditoria', valor: '100%' },
                { label: 'Segurança', valor: '100%' },
                { label: 'Rastreabilidade', valor: '100%' }
              ].map((item, i) => (
                <div key={i} className="text-center">
                  <p className="text-xs text-slate-600 mb-1">{item.label}</p>
                  <p className="text-2xl font-bold text-green-600">{item.valor}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-blue-50 rounded-xl p-4 border-2 border-blue-300">
            <p className="text-sm text-slate-700">
              <strong className="text-green-700">14 Componentes</strong> • 
              <strong className="text-green-700"> 5 Melhorados</strong> • 
              <strong className="text-green-700"> 5.000+ Linhas</strong> • 
              <strong className="text-green-700"> 100% Operacional</strong>
            </p>
          </div>
