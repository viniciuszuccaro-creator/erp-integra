...
export default function DashboardFinanceiroUnificado({ empresaId }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="border-2 border-blue-200">
        <CardHeader className="bg-gradient-to-r from-blue-50 to-indigo-50 border-b">
          <CardTitle className="flex items-center gap-2">
            ...
            Status das Integrações (ETAPA 4)
          </CardTitle>
        </CardHeader>
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold">Caixa Central</p>
                  <p className="text-xs text-slate-600">{ordFiltradas.length} ordens criadas</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600">{pagFiltrados.length} pagamentos</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold">Aprovações Desconto</p>
                  <p className="text-xs text-slate-600">{pedidosAprovacaoPendente} pendentes</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600">IA Pareamento</p>
                </div>
              </div>
      ...
      {/* Alertas */}
      {(ordensPendentes > 0 || pedidosAprovacaoPendente > 0 || pagamentosPendentes > 0) && (
        <Card className="border-orange-300 bg-orange-50">
          <CardContent className="p-6">
            <div className="flex items-start gap-3">
              <AlertCircle className="w-6 h-6 text-orange-600 flex-shrink-0 mt-1" />
              <div className="space-y-2">
                <p className="font-semibold text-orange-900">Ações Pendentes:</p>
                <ul className="text-sm text-orange-800 space-y-1 list-disc ml-5">
                  {ordensPendentes > 0 && (
                    <li>{ordensPendentes} ordem(ns) aguardando liquidação no Caixa Central</li>
                  )}
                  {pedidosAprovacaoPendente > 0 && (
                    <li>{pedidosAprovacaoPendente} pedido(s) aguardando aprovação de desconto</li>
                  )}
                  {pagamentosPendentes > 0 && (
                    <li>{pagamentosPendentes} pagamento(s) omnichannel aguardando conciliação</li>
                  )}
                </ul>
              </div>
            </div>
