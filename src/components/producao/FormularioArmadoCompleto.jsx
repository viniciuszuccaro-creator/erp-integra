...
/**
 * V21.1: FORMULÁRIO ARMADO COMPLETO
 * Agora recebe props isOpen/onClose para ser usado como modal
 */
export default function FormularioArmadoCompleto({ isOpen, onClose, onSubmit, itemInicial = null, obraDestino = null }) {
  ...
  const [formData, setFormData] = useState(itemInicial || {
    ...
    preco_venda_total: 0,
    obra_destino: obraDestino // Added obra_destino
  });
  ...
  const handleSalvar = () => {
    if (!tipoSelecionado) {
      toast.error("Selecione o tipo de elemento estrutural");
      return;
    }

    if (!formData.comprimento) {
      toast.error("Preencha o comprimento");
      return;
    }

    if (tipoSelecionado === "Bloco" && (!formData.altura || !formData.largura)) {
      toast.error("Preencha altura e largura do bloco");
      return;
    }
    ...
    if (elementoObrigatorio && !elementoEstrutural.trim()) {
      toast.error("⚠️ Elemento Estrutural Obrigatório! Este item foi processado por IA e requer o preenchimento do elemento estrutural (ex: V1, V2, C1, B1).");
      return;
    }
    ...
    const itemCompleto = {
      ...
      tipo_servico: "armado",
      tipo_armacao: tipoSelecionado, // Added
    ...
    onSubmit(itemCompleto); // Changed from onSalvar to onSubmit
    toast.success("✅ Item Adicionado ao Pedido!"); // Changed toast call
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Nova Peça Armada {obraDestino && `- ${obraDestino}`}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6">
              ...
              <Card>
                ...
                <CardContent className="p-6 space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    {/* Removed Identificador Interno input from here based on outline */}
                    <div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            <Button type="button" variant="outline" onClick={onClose}> {/* Changed from onCancelar to onClose */}
              Cancelar
            </Button>
