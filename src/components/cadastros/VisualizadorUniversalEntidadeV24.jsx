/**
 * VisualizadorUniversalEntidadeV24 — V25.0
 * ✅ asServiceRole para listar/salvar/deletar (bypassa wrapper Layout)
 * ✅ Editar: busca registro completo via asServiceRole
 * ✅ Exclusão em massa cross-page com deselectedIds
 * ✅ Ordenação via refs (sem stale closure)
 * ✅ Contagem via useEntityCounts
 */
...
import { ChevronUp, ChevronDown, ChevronsUpDown, Search, Edit, Trash2, Plus, RefreshCw, AlertCircle, X } from "lucide-react";
...
const SEARCH_FIELDS_MAP = {
  ...
  Cargo:['nome','departamento'], Turno:['descricao'], Veiculo:['placa','descricao','modelo'],
  Motorista:['nome','cpf'], Servico:['nome','descricao'], Representante:['nome','email'],
  SegmentoCliente:['nome_segmento'], RegiaoAtendimento:['nome_regiao'],
  GrupoProduto:['nome_grupo','descricao','codigo'], Marca:['nome_marca'],
  SetorAtividade:['nome'], TabelaPreco:['nome'], UnidadeMedida:['sigla','descricao'],
  CentroCusto:['codigo','descricao'], PlanoDeContas:['codigo','descricao'],
  TipoDespesa:['codigo','nome'], MoedaIndice:['moeda','indice'],
  GrupoEmpresarial:['nome','cnpj'], Empresa:['razao_social','nome_fantasia','cnpj'],
  ApiExterna:['nome','descricao'], ChatbotCanal:['nome'], ChatbotIntent:['nome','descricao'],
  GatewayPagamento:['nome'], JobAgendado:['nome'], Webhook:['nome','url'],
};
...
async function fetchFullRecord(entityName, itemId) {
  ...
  try {
    ...
    if (api?.filter) {
      ...
      if (Array.isArray(res) && res.length > 0 && res[0]?.id) return JSON.parse(JSON.stringify(res[0]));
    }
  } catch (_) {}
  try {
    const api = base44.asServiceRole?.entities?.[entityName];
    if (api?.get) { const rec = await api.get(itemId); if (rec?.id) return JSON.parse(JSON.stringify(rec)); }
  } catch (_) {}
  try {
...
export default function VisualizadorUniversalEntidadeV24({
  ...
  const isSelf   = FormComponent ? SELF_MANAGED_FORMS.has(FormComponent.displayName || FormComponent.name || "") : false;

  const COLUMNS = useMemo(() => {
    if (columns?.length > 0) return columns;
    if (camposPrincipais?.length > 0) {
      return camposPrincipais.map(campo => ({
        field: campo, label: campo.replace(/_/g," ").replace(/\b\w/g, c => c.toUpperCase()),
        sortable: true, searchable: !STATUS_FIELDS.has(campo) && !BOOL_FIELDS.has(campo),
      }));
    }
    return [{ field:"nome", label:"Nome", sortable:true, searchable:true }, { field:"status", label:"Status", sortable:false, searchable:false }];
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => { setDebouncedSearch(search); setCurrentPage(1); }, 350);
  ...
  }, [ENTITY, empresaAtual?.id, grupoAtual?.id, JSON.stringify(empresasDoGrupo)]); // eslint-disable-line

  const skip = (currentPage - 1) * pageSize;
  const queryKey = useMemo(() => [ENTITY, "viz-v25", sortField, sortDir, currentPage, pageSize, debouncedSearch, empresaAtual?.id, grupoAtual?.id], [ENTITY, sortField, sortDir, currentPage, pageSize, debouncedSearch, empresaAtual?.id, grupoAtual?.id]);
  ...
  const { data: items = [], isLoading, isFetching } = useQuery({
    ...
    queryFn: async () => {
      ...
      try { const result = await api.filter(filter, order, pageSize, skip); return Array.isArray(result) ? result : []; }
      catch (err) { console.error(`[VizV25] Erro ao listar ${ENTITY}:`, err); return []; }
    },
    staleTime: 15_000, gcTime: 120_000, refetchOnWindowFocus: false,
  ...
  const handleSort = useCallback((field) => {
    const cur = sortFieldRef.current; const curDir = sortDirRef.current;
    const newDir = cur === field ? (curDir === "desc" ? "asc" : "desc") : "desc";
    setSortField(field); setSortDir(newDir); setCurrentPage(1);
    queryClient.invalidateQueries({ queryKey: [ENTITY, "viz-v25"] });
  }, [ENTITY, queryClient, setSortField, setSortDir]);

  const handleSortDropdown = useCallback((value) => {
    const idx = value.lastIndexOf("|"); if (idx === -1) return;
    const f = value.slice(0, idx); const d = value.slice(idx + 1);
  ...
  const formatValue = useCallback((value, col) => {
    ...
    if (BOOL_FIELDS.has(col.field)) return value ? <Badge variant="outline" className="text-xs rounded-sm bg-green-100 text-green-700 border-green-300">Sim</Badge> : <Badge variant="outline" className="text-xs rounded-sm bg-slate-100 text-slate-500 border-slate-300">Não</Badge>;
    if (STATUS_FIELDS.has(col.field) && typeof value === "string") { const cls = statusColors[value] || "bg-slate-100 text-slate-600"; return <Badge variant="outline" className={`text-xs rounded-sm ${cls}`}>{value}</Badge>; }
    if (DATE_FIELDS.has(col.field) || col.type === "date") { try { const d = new Date(value); if (!isNaN(d.getTime())) return d.toLocaleDateString("pt-BR"); } catch {} }
    if (MONEY_FIELDS.has(col.field) || col.type === "currency") { const n = Number(value); if (!isNaN(n)) return n.toLocaleString("pt-BR", { style:"currency", currency:"BRL", minimumFractionDigits:2 }); }
  ...
  const handlePersistSubmit = useCallback(async (formData) => {
    ...
    if (formData._action === "delete") {
      if (formData.id) { try { await (base44.asServiceRole?.entities?.[ENTITY] || base44.entities[ENTITY]).delete(formData.id); } catch (_) {} }
      handleSave(); return;
    }
    ...
    try {
      ...
      if (editItem?.id) { await api.update(editItem.id, payload); } else { await api.create(payload); }
      handleSave();
  ...
  const handleEditItem = useCallback(async (item) => {
    if (!item?.id) return;
    setIsLoadingEdit(true); setShowForm(false); setEditItem(null); setEditError(null);
    try {
      const full = await fetchFullRecord(ENTITY, item.id);
      setFormKey(k => k + 1);
      if (full) { setEditItem(full); } else { setEditError("Dados parciais."); setEditItem(JSON.parse(JSON.stringify(item))); }
      setShowForm(true);
    } catch (err) { setFormKey(k => k + 1); setEditItem(JSON.parse(JSON.stringify(item))); setShowForm(true); }
  ...
  const handleDeleteSelected = useCallback(async () => {
    const effectiveCount = crossPageAll ? Math.max(0, totalCount - deselectedIds.size) : selectedIds.size;
    if (effectiveCount === 0) return;
    const msg = crossPageAll ? `⚠️ Confirma exclusão de ${effectiveCount} registros? Irreversível!` : `Confirma exclusão de ${selectedIds.size} registro(s)?`;
    if (!window.confirm(msg)) return;
    try {
      let idsToDelete = [];
      const api = base44.asServiceRole?.entities?.[ENTITY];
      if (crossPageAll && api?.filter) {
        let skipAcc = 0; const BATCH = 500;
        while (true) {
          const arr = await api.filter(contextFilter, '-updated_date', BATCH, skipAcc) || [];
          idsToDelete = idsToDelete.concat(arr.map(i => i.id).filter(id => id && !deselectedIds.has(id)));
      ...
      for (let i = 0; i < idsToDelete.length; i += CONC) {
        ...
        await Promise.all(batch.map(id => { const a = base44.asServiceRole?.entities?.[ENTITY] || base44.entities[ENTITY]; return a.delete(id).catch(() => {}); }));
      }
      queryClient.invalidateQueries({ queryKey: [ENTITY, "viz-v25"] });
      queryClient.invalidateQueries({ queryKey: ["entityCounts_v4"] });
      setSelectedIds(new Set()); setDeselectedIds(new Set()); setCrossPageAll(false); setCurrentPage(1);
    } catch (e) { alert("Erro ao excluir: " + (e?.message || e)); }
  }, [crossPageAll, totalCount, selectedIds, deselectedIds, ENTITY, queryClient, contextFilter]);
  ...
  const handleItemCheck = useCallback((id, checked) => {
    if (crossPageAll) { setDeselectedIds(prev => { const n = new Set(prev); checked ? n.delete(id) : n.add(id); return n; }); }
    else { setSelectedIds(prev => { const n = new Set(prev); checked ? n.add(id) : n.delete(id); return n; }); }
  }, [crossPageAll]);
  ...
  const toggleSelectAll = useCallback(() => {
    if (crossPageAll) { setDeselectedIds(prev => { const n = new Set(prev); if (allPageSelected) { items.forEach(i => n.add(i.id)); } else { items.forEach(i => n.delete(i.id)); } return n; }); }
    else if (allPageSelected) { setSelectedIds(prev => { const n = new Set(prev); items.forEach(i => n.delete(i.id)); return n; }); }
  ...
  const getSortIcon = (field) => {
    ...
    return sortDir === "desc" ? <ChevronDown className="w-3.5 h-3.5 text-blue-500" /> : <ChevronUp className="w-3.5 h-3.5 text-blue-500" />;
  };

  const effectiveSelectedCount = crossPageAll ? Math.max(0, totalCount - deselectedIds.size) : selectedIds.size;
  const showCrossPageBanner = !crossPageAll && selectedIds.size > 0 && allPageSelected && totalCount > items.length;

  const formProps = useMemo(() => buildFormProps(editItem, handleSave, isSelf ? null : handlePersistSubmit), [formKey, handleSave, handlePersistSubmit, isSelf]); // eslint-disable-line
    ...
    <div className="flex flex-col h-full gap-3 min-h-0">
      ...
      <div className="flex items-center gap-2 flex-wrap shrink-0">
        <div className="flex items-center gap-1 shrink-0">
          ...
          <Badge variant="outline" className="rounded-sm bg-blue-50 text-blue-700 border-blue-200 font-bold">{totalCount}</Badge>
        </div>
        <div className="relative flex-1 min-w-[160px]">
          <Search className="absolute left-3 top-2.5 w-3.5 h-3.5 text-slate-400 pointer-events-none" />
          <Input placeholder={`Buscar ${TITULO}...`} value={search} onChange={e => setSearch(e.target.value)} className="pl-8 bg-white border-slate-200 h-9 rounded-sm text-sm" />
        </div>
        <select value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); setCurrentPage(1); }} className="border border-slate-200 rounded-sm h-9 px-2 text-sm text-slate-700 bg-white cursor-pointer">
          {PAGE_SIZES.map(ps => <option key={ps} value={ps}>{ps} / pág.</option>)}
        </select>
        <select value={`${sortField}|${sortDir}`} onChange={e => handleSortDropdown(e.target.value)} className="border border-slate-200 rounded-sm h-9 px-2 text-sm text-slate-700 bg-white cursor-pointer" title="Ordenação">
        ...
        <button type="button" onClick={() => { queryClient.invalidateQueries({ queryKey: [ENTITY, "viz-v25"] }); queryClient.invalidateQueries({ queryKey: ["entityCounts_v4"] }); }} className="h-9 w-9 flex items-center justify-center border border-slate-200 rounded-sm bg-white hover:bg-slate-50 transition-colors" title="Recarregar">
          <RefreshCw className={`w-4 h-4 ${isFetching ? "animate-spin text-blue-500" : "text-slate-500"}`} />
        </button>
        {FormComponent && (
          <Button size="sm" onClick={() => { setFormKey(k => k + 1); setEditItem(null); setEditError(null); setShowForm(true); }} className="h-9 rounded-sm gap-1" disabled={isLoadingEdit}>
        ...
        {effectiveSelectedCount > 0 && (
          <Button size="sm" variant="destructive" onClick={handleDeleteSelected} className="h-9 rounded-sm gap-1">
            ...
            {crossPageAll ? `Apagar ${effectiveSelectedCount === totalCount ? "TODOS" : effectiveSelectedCount} (${effectiveSelectedCount})` : `Apagar ${selectedIds.size}`}
          </Button>
        )}
      </div>
      ...
      {showCrossPageBanner && (
        <div className="bg-amber-50 border border-amber-200 rounded-sm px-3 py-2 text-sm text-amber-700 flex items-center gap-2 shrink-0">
          ...
          <button onClick={() => { setCrossPageAll(true); setDeselectedIds(new Set()); setSelectedIds(new Set()); }} className="text-blue-600 hover:text-blue-800 underline text-xs font-semibold">Selecionar todos os {totalCount} registros</button>
      ...
      {crossPageAll && (
        <div className="bg-blue-50 border border-blue-200 rounded-sm px-3 py-2 text-sm text-blue-700 flex items-center gap-2 shrink-0">
          <span>✓ {deselectedIds.size > 0 ? `${effectiveSelectedCount} de ${totalCount} selecionados (${deselectedIds.size} desmarcado${deselectedIds.size > 1 ? 's' : ''})` : `Todos os ${totalCount} registros de ${TITULO} selecionados`}</span>
          <button onClick={() => { setCrossPageAll(false); setDeselectedIds(new Set()); setSelectedIds(new Set()); }} className="ml-auto text-blue-500 hover:text-blue-700 underline text-xs">Cancelar seleção</button>
        </div>
      )}

      <div className="flex-1 overflow-auto rounded-sm border border-slate-200 bg-white">
        {isLoading && items.length === 0 ? (
          <div className="space-y-1.5 p-4">{[...Array(8)].map((_, i) => <Skeleton key={i} className={`h-8 rounded-sm ${i % 3 === 0 ? "w-3/4" : "w-full"}`} />)}</div>
        ) : items.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-slate-400 gap-2">
            ...
            {needsContextWarning && <span className="text-xs text-amber-600 flex items-center gap-1"><AlertCircle className="w-3.5 h-3.5" /> Selecione uma empresa ou grupo</span>}
          </div>
        ) : (
            ...
            <thead className="sticky top-0 bg-slate-50 border-b border-slate-200 z-10">
              <tr>
                <th className="px-4 py-2.5 text-center w-10">
                  <input type="checkbox" ref={el => { if (el) el.indeterminate = somePageSelected && !allPageSelected; }} checked={allPageSelected} onChange={toggleSelectAll} className="w-4 h-4 cursor-pointer" />
                </th>
                {COLUMNS.map(col => (
                  <th key={col.field} className={`group/th px-4 py-2.5 text-left font-semibold text-slate-600 select-none whitespace-nowrap ${col.sortable !== false ? "cursor-pointer hover:bg-slate-100 transition-colors" : ""}`} onClick={() => col.sortable !== false && handleSort(col.field)}>
                    <div className="flex items-center gap-1"><span>{col.label}</span>{col.sortable !== false && getSortIcon(col.field)}</div>
                  </th>
                ))}
              ...
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className={`hover:bg-blue-50/30 transition-colors group/row ${checked ? "bg-blue-50/40" : ""}`}>
                    <td className="px-4 py-2 text-center"><input type="checkbox" checked={checked} onChange={e => handleItemCheck(item.id, e.target.checked)} className="w-4 h-4 cursor-pointer" /></td>
                    {COLUMNS.map(col => <td key={col.field} className="px-4 py-2 text-slate-600 max-w-[280px] truncate">{formatValue(item[col.field], col)}</td>)}
                    <td className="px-4 py-2">
                      <div className="flex items-center justify-center gap-1 opacity-60 group-hover/row:opacity-100 transition-opacity">
                        {FormComponent && (
                          <button type="button" onClick={e => { e.stopPropagation(); handleEditItem(item); }} title="Editar" disabled={isLoadingEdit} className="h-7 w-7 flex items-center justify-center rounded-sm text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors disabled:opacity-40">
                            {isLoadingEdit ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Edit className="w-3.5 h-3.5" />}
                          </button>
                        )}
                        <button type="button" onClick={() => handleDelete(item)} title="Excluir" className="h-7 w-7 flex items-center justify-center rounded-sm text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                );
      ...
      <div className="flex items-center justify-between text-xs text-slate-500 shrink-0 flex-wrap gap-2">
        ...
        <div className="flex gap-1.5">
          <button type="button" onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1 || isLoading} className="h-7 px-2 text-xs border border-slate-200 rounded-sm bg-white hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed">← Anterior</button>
          <span className="flex items-center justify-center h-7 min-w-7 px-2 border border-slate-200 rounded-sm bg-white text-slate-700 font-medium">{currentPage}</span>
          <button type="button" onClick={() => setCurrentPage(p => p + 1)} disabled={items.length < pageSize || isLoading} className="h-7 px-2 text-xs border border-slate-200 rounded-sm bg-white hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed">Próxima →</button>
        </div>
      </div>
      ...
      {FormComponent && showForm && (
        <>
          ...
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 pointer-events-none">
            <div className="bg-white rounded-xl w-full max-w-4xl max-h-[92vh] overflow-auto shadow-2xl pointer-events-auto flex flex-col" onClick={e => e.stopPropagation()}>
              <div className="sticky top-0 bg-white border-b px-6 py-4 flex items-center justify-between z-10 rounded-t-xl">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button type="button" onClick={handleSave} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors"><X className="w-5 h-5" /></button>
                </div>
              </div>
              <div className="p-6 flex-1 overflow-auto">
                <FormComponent key={`form-${ENTITY}-${editItem?.id || 'new'}-${formKey}`} {...formProps} />
              </div>
            </div>
          </div>
        </>
  ...
  if (windowMode) {
    return (
      <div className="w-full h-full flex flex-col p-4 bg-white overflow-hidden">
        {IconeProp && <div className="flex items-center gap-2 mb-3 shrink-0"><IconeProp className="w-5 h-5 text-slate-500" /><h2 className="text-base font-semibold text-slate-800">{TITULO}</h2></div>}
