...
export default function MinimizedWindowsBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-slate-800 to-slate-900 border-t-2 border-blue-500/50 p-3 flex gap-3 z-[9999] shadow-2xl backdrop-blur-sm">
      <div className="flex items-center gap-2 px-3 text-slate-300 border-r border-slate-600">
        ...
        <span className="text-sm font-medium">minimizada{minimizedWindows.length > 1 ? 's' : ''}</span>
      </div>
      ...
      <AnimatePresence>
        {minimizedWindows.map(window => (
          <motion.button
            ...
            className="group relative flex items-center gap-2 bg-slate-700 hover:bg-slate-600 text-white px-4 py-2.5 rounded-lg transition-all shadow-lg hover:shadow-xl hover:scale-105 max-w-xs border border-slate-600 hover:border-blue-500"
          >
